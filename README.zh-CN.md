...
Python package 只安装 `sidecar` CLI 和 `sidecar-mcp` entry points。Skill asset 独立位于 `sidecar-manager-skill/SKILL.md`；把该目录安装或复制到当前 Claude Code 环境使用的 skill 目录后，以 `sidecar-manager` 调用。Skill 只是操作者工作流：它选择安全命令、解释取舍，但不替代 CLI safety gates。
