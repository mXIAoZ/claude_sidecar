...
- `compact_sidecar.runtime.merge_compact_history` 从最近 history 中提取、去重 summary，生成 `rolling-summary.draft.md`，但不自动覆盖人工维护的 `rolling-summary.md`。
- `compact_sidecar.services.daemon` 支持 run-once、bounded foreground loop、默认 LLM 生成并写入 `rolling-summary.md`、launchd plist artifact 管理、read-only doctor 和显式确认的 launchctl lifecycle。
- `compact_sidecar.ui.dashboard` / status 路径提供只读健康视图、compact-readiness 近似信号、LLM token usage 和 operation timeline，默认隐藏 raw prompt/summary。
- `compact_sidecar.services.auto_compact_controller` 作为 hook 外层显式 tmux controller，提供 `--pane` 时才会发送 `/compact` 或 prompt，`--no-send` 只输出计划；`--merge-after` 会先备份旧 `rolling-summary.md`，再直接写入新的 `rolling-summary.md`。
- `compact_sidecar.cli` 提供统一 CLI，把 hook setup、daemon startup、auto compact 和状态查看聚合到一个入口，同时保留原有安全 gate。
- 真实实现位于 `src/compact_sidecar/` 包中，按 `hooks/`、`runtime/`、`services/`、`ui/` 和 `mcp/` 分组；source checkout 使用 `PYTHONPATH=src python3 -m compact_sidecar...`，安装后使用 `sidecar` / `sidecar-mcp`。
...
```bash
PYTHONPATH=src python3 -m compact_sidecar.cli setup --settings /tmp/sidecar-settings.json --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.cli status --json
PYTHONPATH=src python3 -m compact_sidecar.cli start daemon --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.cli start compact --pane session:window.pane --prompt-file /path/to/prompt.txt --wait-postcompact --merge-after
```

`compact_sidecar.cli` 是统一 CLI 入口。它不会改变 hook 能力边界：`UserPromptSubmit` 仍然只在 prompt submit 时注入 `additionalContext`。自动 compact flow 的受支持语义是 controller 发送 `/compact`，可选等待 `PostCompact`，可选先备份旧 `rolling-summary.md` 再写入新的 `rolling-summary.md`，然后发送 prompt，由这次 prompt submit 触发 `UserPromptSubmit` 注入。`sidecar setup` / `python -m compact_sidecar.cli setup` 默认可以写真实 `~/.claude/settings.json`；用 `--settings` 写临时文件，用 `--no-launchctl` 跳过 launchd 启动，用 `--no-send` 跳过 tmux 发送。tmux 发送仍必须显式提供 `--pane`。
...
- 真实 sidecar 操作必须由项目 CLI / daemon / controller 执行：hook 配置走 `sidecar setup` / `python -m compact_sidecar.cli setup` 或 `python -m compact_sidecar.hooks.install`，daemon lifecycle 走 `sidecar start daemon` / `python -m compact_sidecar.cli start daemon` 或 `python -m compact_sidecar.services.daemon`，auto compact 走 `sidecar start compact` / `python -m compact_sidecar.cli start compact` 或 `python -m compact_sidecar.services.auto_compact_controller`。
...
```bash
PYTHONPATH=src python3 -m compact_sidecar.hooks.userprompt
```
...
```bash
PYTHONPATH=src python3 -m compact_sidecar.hooks.postcompact
```
...
```bash
PYTHONPATH=src python3 -m compact_sidecar.ui.status
```

`compact_sidecar.ui.status` 是 run-once 诊断命令，只读取当前项目 `.memory/` 中已知文件并输出状态；它不写入 `errors.log`，不创建目录，不修改 `rolling-summary.md`，不编辑 `~/.claude/settings.json`，不启动 daemon，不扫描 transcript 或源码。它还输出 `compact-readiness` 近似信号：该信号只基于本地 runtime 文件的字符数/字节数 metadata，不能代表 Claude Code 内部精确 token 使用率，也不会自动触发 compact。

本地 daemon run-once / loop 命令：

```bash
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --run-once
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --loop --interval-seconds 300
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --loop --interval-seconds 1 --max-runs 2
```

`compact_sidecar.services.daemon --run-once` 只执行一次维护：从 compact history 生成 `rolling-summary.draft.md`，并在有候选 summary 时默认调用环境变量配置的 OpenAI-compatible LLM，校验输出后 backup-first 写入 `rolling-summary.md`，同时写入 metadata-only 的 `daemon-state.json`。`--loop` 在前台按间隔重复执行同一维护逻辑；测试和 smoke check 应使用 `--max-runs` 保证退出。它不会扫描 transcript/source，也不会编辑真实 Claude settings。
...
```bash
PYTHONPATH=src python3 -m compact_sidecar.services.auto_compact_controller --pane session:window.pane --prompt-file /path/to/prompt.txt
PYTHONPATH=src python3 -m compact_sidecar.services.auto_compact_controller --pane session:window.pane --prompt-file /path/to/prompt.txt --wait-postcompact --merge-after
```

`compact_sidecar.services.auto_compact_controller` 是 hook 外层的显式会话控制器。它必须提供 `--pane` 且未传入 `--no-send` 才会通过 tmux 发送内容；controller 不猜测当前 tmux pane，不读取 shell history，不通过 shell 字符串执行命令。它用近似 readiness 判断是否先发送 `/compact`，可选用 `--wait-postcompact` 等待 `compact-history.jsonl` metadata 变化；启用 `--merge-after` 时会从 compact history 生成新的 `rolling-summary.md`，并先把旧 summary 保存为 `rolling-summary.backup.<date>.md`。随后 controller 发送原 prompt。prompt 文本只能来自 `--prompt-file` 或 `--prompt-stdin`，不会写入 `.memory/`、日志、stdout/stderr 或 state。
...
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --doctor --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --remove-agent --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --launchctl-bootstrap --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --launchctl-kickstart --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --launchctl-status --plist-path /tmp/sidecar.plist
PYTHONPATH=src python3 -m compact_sidecar.services.daemon --launchctl-bootout --plist-path /tmp/sidecar.plist
```
...
```bash
PYTHONPATH=src python3 -m compact_sidecar.hooks.install --settings /tmp/sidecar-settings.json
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "PYTHONPATH=/absolute/path/to/claude_code_compact_sidecar/src python3 -m compact_sidecar.hooks.userprompt",
            "timeout": 5,
    ...
    "PostCompact": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "PYTHONPATH=/absolute/path/to/claude_code_compact_sidecar/src python3 -m compact_sidecar.hooks.postcompact",
            "timeout": 5,
...
产品化后的仓库同时支持三种入口：source checkout module commands、packaged console entry points、Claude Code Skill/MCP 辅助层。
...
- Skill 中的真实写操作必须调用 `compact_sidecar.cli`、`compact_sidecar.services.daemon`、`compact_sidecar.hooks.install` 或 `compact_sidecar.services.auto_compact_controller` 的现有门禁；source checkout 使用 `PYTHONPATH=src python3 -m ...`。
- 默认 settings target 是项目本地 `.claude/settings.local.json`；全局 `~/.claude/settings.json` 必须由用户明确要求。

MCP 契约：

- `src/compact_sidecar/mcp/server.py` 使用标准库实现 stdio JSON-RPC，支持 `initialize`、`tools/list` 和 `tools/call`。
...
- hook uninstall 通过 `sidecar uninstall` / `python -m compact_sidecar.cli uninstall` 或 `python -m compact_sidecar.hooks.install --uninstall` 类入口执行，不能直接覆盖 settings 文件。
...
源码目录用于开发、测试和 module entrypoint 执行；source checkout 入口统一使用 `PYTHONPATH=src python3 -m compact_sidecar...`。`.memory/` 只保存当前项目的用户 summary、compact history、operation log 和 hook/daemon 日志，避免把运行时数据写入全局目录。

文件职责：

- `compact_sidecar.hooks.userprompt`：读取 rolling summary，输出 `UserPromptSubmit` hook JSON，通过 `additionalContext` 注入。
- `compact_sidecar.runtime.summary_context`：共享 rolling summary 读取、空值处理和 head/tail 截断逻辑。
- `compact_sidecar.hooks.postcompact`：可选，记录 `PostCompact` payload，便于用户之后整理 summary。
- `compact_sidecar.runtime.merge_compact_history`：从 compact history 生成 `rolling-summary.draft.md`，供用户手动审查。
- `compact_sidecar.services.daemon`：支持 `--run-once`、有界 foreground `--loop`、默认 LLM 写入 rolling summary、launchd plist 生成、plist artifact 只读检查和显式安全移除；artifact 命令不调用 `launchctl`。显式 `--launchctl-*` 命令可在通过 plist 校验后调用 launchctl 管理用户级 launchd state。
- `compact_sidecar.services.llm_summarizer`：只用标准库发送 OpenAI-compatible streaming chat completions 请求，解析 SSE chunks 中的 `choices[].delta.content` 和 usage token，并保证错误消息不泄漏 API key value。
- `compact_sidecar.runtime.rolling_summary_writer`：集中校验 `# Rolling Summary` 和 `## Compact 前必须保留`，并用 backup-first + atomic replace 写入 `rolling-summary.md`。
- `compact_sidecar.services.auto_compact_controller`：hook 外层的显式 tmux controller；提供 `--pane` 时才会发送 `/compact` 或 prompt，`--no-send` 只输出计划，可选等待 `PostCompact` history 变化；启用 `--merge-after` 时先备份旧 `rolling-summary.md`，再写入新的 `rolling-summary.md`。默认不保存 prompt 文本。
- `compact_sidecar.runtime.operation_log`：写入、读取、轮转和检查 project-local operation timeline；logging failure 必须 best-effort，不阻断 hook/controller/daemon。
- `compact_sidecar.ui.dashboard`：只读终端 Dashboard，展示 runtime health、compact-readiness、known files、recent operations 和 warnings；默认隐藏 raw content。
- `compact_sidecar.cli`：统一 CLI，封装 hook 安装、daemon 启动、auto compact controller 和 read-only status；只复用现有安全 gate，不绕过确认。
- `compact_sidecar.hooks.install`：把所需 Claude Code hooks 安全合并到 `settings.json`，保留既有配置并避免重复安装。
- `compact_sidecar.ui.status`：run-once 诊断命令，只读取当前项目 `.memory/` 中已知文件并输出状态；它不写入 `errors.log`，不创建目录，不修改 `rolling-summary.md`，不编辑 `~/.claude/settings.json`，不启动 daemon，不扫描 transcript 或源码。它还输出基于本地 runtime 文件大小/字符数 metadata 的近似 `compact-readiness`，不保存 prompt/transcript 内容，不代表精确 token 使用率，也不自动触发 compact。
...
- `daemon-state.json`：`compact_sidecar.services.daemon` 写入的本地状态文件，只包含时间、模式、候选数量、draft 路径、LLM summary status、provider/model、token usage、耗时、summary/backup path、plist path、launchctl_invoked、launchctl_action、launchctl_target、launchctl_returncode、launchctl_status、plist_validated、error_kind、loop interval/run count/shutdown reason 等 metadata，不保存 summary 原文、LLM prompt/output 原文、API key value、plist XML 或 launchctl stdout/stderr 原文。
...
`compact_sidecar.hooks.userprompt` 行为：
...
`compact_sidecar.runtime.operation_log` / `compact_sidecar.ui.dashboard` 行为：
...
- raw prompt 只允许 `compact_sidecar.services.auto_compact_controller --operation-log --log-raw-prompt` 显式记录。
- raw summary 只允许 `SIDECAR_LOG_RAW_SUMMARY=1` 或 `compact_sidecar.runtime.merge_compact_history --operation-log --log-raw-summary` 显式记录。
- `compact_sidecar.ui.dashboard` 默认一次性只读渲染；`--watch` 循环刷新；`--json` 输出 machine-readable snapshot；`--show-content` 是显示 raw prompt/summary 的唯一开关；LLM token metadata 默认可显示，因为它不包含 raw prompt/summary。
- `compact_sidecar.ui.status` 和 dashboard 默认输出都不能打印 raw prompt/summary；只能显示 raw-content flags 和 hidden marker。
- malformed operation log 只能影响 read-only status/dashboard warnings，不能写 `errors.log`。

`compact_sidecar.hooks.postcompact` 行为：
...
`compact_sidecar.runtime.merge_compact_history` 行为：
...
- 手动 `compact_sidecar.runtime.merge_compact_history` 不自动覆盖 `rolling-summary.md`；用户必须手动审查 draft，只复制仍然准确且值得长期保留的信息。
- 如果 history 缺失或没有 summary，仍生成一个空 draft 模板。

`compact_sidecar.services.daemon --run-once` / `--loop` / `--install-agent` / `--agent-status` / `--remove-agent` 行为：

- `--run-once` 从 compact history 收集最近 summary 候选，复用 `compact_sidecar.runtime.merge_compact_history` 的 draft 格式写入兼容 draft。
...
- `--install-agent --plist-path <path>` 只写 plist 文件和 metadata-only daemon state；ProgramArguments 指向 `python -m compact_sidecar.services.daemon --loop --interval-seconds N --operation-log`，WorkingDirectory 固定为当前项目根，EnvironmentVariables 固定 `SIDECAR_COMPACT_DIR`，并携带非 secret 的 `SIDECAR_LLM_*` 设置和 API key 环境变量名，不携带解析后的 API key value，stdout/stderr 日志路径位于 runtime dir。
...
- `rolling-summary.md` 不存在时，`compact_sidecar.hooks.userprompt` 输出有效 no-op JSON `{}`。
- `rolling-summary.md` 为空时，`compact_sidecar.hooks.userprompt` 输出有效 no-op JSON `{}`。
- `rolling-summary.md` 有内容但没有 `## Compact 前必须保留` marker 时，输出有效 no-op JSON `{}`。
- 有 marker 或设置 `SIDECAR_INJECT_ALWAYS=1` 时，`compact_sidecar.hooks.userprompt` 输出包含 `additionalContext` 的有效 JSON。
- `rolling-summary.md` 超过大小上限时，输出“开头 + 截断提示 + 结尾”的内容和整理提示。
- 输出可以通过 `python3 -m json.tool` 校验。
- `compact_sidecar.hooks.postcompact` 能接收合成 JSON，拒绝超过 200k 字符的 payload，并写入 `compact-history.jsonl`。
- `compact_sidecar.services.llm_summarizer` request 测试使用 fake streaming `urllib.request.urlopen`，验证 request body/header、usage token 解析、无 usage 时 token unknown、错误不泄漏 API key value，禁止真实网络。
- `compact_sidecar.runtime.rolling_summary_writer` 测试验证 required heading/marker、backup-first 写入和新文件写入。
- `compact_sidecar.services.daemon --run-once` 无 history 时跳过 LLM 且不覆盖 `rolling-summary.md`；有 history 和 fake LLM 时写入 `rolling-summary.md`、创建旧 summary backup、记录 token metadata。
- `compact_sidecar.services.daemon --run-once` 在 LLM 失败时不覆盖旧 `rolling-summary.md`，并返回非零。
- `compact_sidecar.services.daemon --loop --max-runs` 能有界退出，更新 metadata-only daemon state，并在单轮 LLM 失败后继续。
- `compact_sidecar.services.daemon --install-agent --plist-path <path>` 只写 plist，不调用 `launchctl`；缺少 `--plist-path` 时安全失败。
- `compact_sidecar.services.daemon --agent-status --plist-path` 能只读检查 plist artifact，缺失/损坏文件不会创建 runtime 或 traceback。
- `compact_sidecar.services.daemon --remove-agent --plist-path` 只删除匹配 sidecar label 的显式 plist artifact，保留 malformed 或非 sidecar plist。
- `compact_sidecar.api` facade 测试覆盖只读调用不写 runtime、默认脱敏、hook setup preview、daemon status 和受控 mutation entrypoints。
- `compact_sidecar.mcp.server` 协议测试覆盖 initialize、tools/list、tools/call、未知工具、参数错误、默认脱敏和 API key value 不泄漏。
...
- 模块级测试全部使用临时 `SIDECAR_COMPACT_DIR`，不会触碰真实 `.memory/` 或 `~/.claude/settings.json`。
- `compact_sidecar.hooks.install` 测试必须通过 `--settings` 指向临时 `settings.json`，不能修改真实 Claude Code 设置。
...
PY
printf '{"session_id":"next"}' | SIDECAR_COMPACT_DIR="$tmp" PYTHONPATH=src python3 -m compact_sidecar.hooks.postcompact
ls -lh "$tmp"
python3 -m json.tool "$tmp/compact-history.jsonl"
```
...
```bash
tmp=$(mktemp -d)
SIDECAR_COMPACT_DIR="$tmp" PYTHONPATH=src python3 -m compact_sidecar.services.daemon --run-once
...
```bash
tmp=$(mktemp -d)
SIDECAR_COMPACT_DIR="$tmp" PYTHONPATH=src python3 -m compact_sidecar.services.daemon --loop --interval-seconds 1 --max-runs 2
python3 -m json.tool "$tmp/daemon-state.json"
test ! -f "$tmp/rolling-summary.md"
```
...
PY
SIDECAR_COMPACT_DIR="$tmp/runtime" PYTHONPATH=src python3 -m compact_sidecar.services.daemon --remove-agent --plist-path "$tmp/not-sidecar.plist" || true
test -f "$tmp/not-sidecar.plist"
```

2. 运行 `PYTHONPATH=src python3 -m compact_sidecar.hooks.userprompt`，确认输出 JSON 有 `additionalContext`，并且包含该 marker。
...
EOF
PYTHONPATH=src python3 -m compact_sidecar.hooks.userprompt | python3 -m json.tool
```

然后运行 `PYTHONPATH=src python3 -m compact_sidecar.hooks.install --settings "$tmp/settings.json"` 检查 settings 合并结果；确认无误后再运行 `PYTHONPATH=src python3 -m compact_sidecar.hooks.install` 安装 `UserPromptSubmit` / `PostCompact` hooks，发送普通 prompt，并询问模型是否记得 `SIDE_CAR_TEST_MARKER_12345`。这个测试是 MVP 最重要的有效性判断：脚本测试只能证明 JSON 输出正确，marker 测试才能证明 hook 流程确实吸收了 sidecar summary。
...
- 可选 `compact_sidecar.hooks.postcompact` 能记录 compact 后 payload。
