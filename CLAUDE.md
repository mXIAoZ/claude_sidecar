...
- Test launchctl lifecycle with fake launchctl only: `python3 -m unittest tests.test_daemon`
...
- `src/daemon.py` supports `--run-once`, bounded foreground `--loop`, launchd plist generation with `--install-agent`, read-only plist artifact inspection with `--agent-status`, explicit safe artifact removal with `--remove-agent`, and explicit `--launchctl-* --confirm-launchctl` lifecycle commands. Artifact modes do not call `launchctl`; only the gated launchctl modes can change user-level launchd state.
...
- `src/daemon.py --run-once` and bounded `--loop --max-runs` are allowed to write draft/state files under `.memory` or `SIDECAR_COMPACT_DIR`, and may write daemon-scoped parse/read failures to `errors.log` with `service=daemon`; `--install-agent --dry-run` writes nothing, `--install-agent --plist-path <path>` only writes a plist/state, `--agent-status --plist-path <path>` is read-only, and `--remove-agent --plist-path <path>` only removes a valid generated sidecar plist artifact with matching label, daemon loop arguments, runtime env, and safe launch flags. These artifact commands do not invoke `launchctl`. Only explicit `--launchctl-bootstrap`, `--launchctl-kickstart`, `--launchctl-status`, and `--launchctl-bootout` with `--confirm-launchctl` may invoke `launchctl`; tests must use `SIDECAR_LAUNCHCTL_PATH` with a fake binary.
